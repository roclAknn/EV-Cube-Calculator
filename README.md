...
・数値入力には四則演算を使用できます('26/03/18)<br>
・スコア入力の１文字目を+-*/にすると自動入力値への演算として扱われます('26/03/18)<br>
・ユニキューブを選択した場合、潜在行数の項目はターゲットとする潜在行数に変更されます<br>
・ユニキューブは行選択率を含めた出力になっています（1行狙いの場合、出力の全数は1/3）<br>
・ヘキサキューブは重くなる可能性があるためスコアの反映を遅らせています<br>
