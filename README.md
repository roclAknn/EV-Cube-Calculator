...
・デザインの調整<br>
---- '26/03 ---------------<br>
・数値入力に四則演算を使用できる機能追加<br>
・スコア入力の１文字目を+-*/にすると自動入力値への演算として扱う機能の追加<br>
・以前の確率出力から、キューブの個数を設定してその個数で出る確率に変更<br>
・出力に昇級確率を表示するように変更（一般の出力に昇級は考慮されません）<br>
・ヘキサキューブの計算速度の改善（依然重めなので出力ディレイはそのまま）<br>
・一部計算精度の上昇<br>
・軽微な不具合修正とデザインの調整<br>
...
<pre>
  TMS公式の確率表を参考にしています
  潜在行数の項目が、**ターゲットとする潜在行数**に変更されます
  行選択率を含めた出力になっています（1行狙いの場合、確率の全数は1/3）
</pre>
