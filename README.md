...
#### 最近の変更
---- '26/05 ---------------<br>
・（**重要**・'26/03/25発生・'26/05/13修正）キューブ切り替え時に、潜在確率テーブルが変更されないことがあった問題の修正<br>
・（**重要**・同上）上記エラーに伴って右下の表出力が中断され、更新されないことがあった問題の修正<br>
・250武器（及び武器系統で201レベル以上を入力した時）の効果量が一部間違っていた問題の修正（確率への影響はなし）<br>
・潜在行数が3行に固定される改変に伴い、潜在行数の設定項目を非表示、及び使用しないように変更<br>
---- '26/04 ---------------<br>
・（**重要**・'26/03/25発生・'26/04/02修正）レッド↔ブラックの切り替え時に各行の上級潜在確率テーブルが変更されていなかった問題の修正<br>
・式入力されたスコアがキューブ切り替え時に引き継がれなかった問題の修正<br>
・ヘッダーの変更、表示の調整機能を追加<br>
・デザインの調整<br>
...
<pre>
  無料キューブテーブルを採用しています
  計算が重くなる可能性があるためスコアの反映を遅らせています
</pre>
・ユニキューブ
<pre>
  TMS公式の確率表を参考にしています
  選択時に設定項目が追加されます（ターゲットとする潜在行数の指定）
  出力は行選択率を含めています（1行狙いの場合、確率の全数は1/3）
</pre>
・装備等級の上昇や、元の潜在能力は出力に反映されません<br>
・潜在確率表上方のつまみを操作すると重要でない潜在能力の表示量が３段階に切り替わります<br>
・数値入力には四則演算を使用できます<br>
・スコア入力の１文字目を+-*/にすると自動入力値への演算として扱われます<br>
・同じスコアをまとめることで軽量化しているため、スコアを細かく入力するほど重くなります<br>
